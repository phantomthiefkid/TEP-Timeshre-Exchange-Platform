...
import { FaChevronLeft, FaChevronRight, FaComment, FaEye, FaStar, FaTrash } from 'react-icons/fa';
...
const FeedbackResortDetail = () => {
    ...
    const handleDeactiveFeedback = async (feedbackId) => {
        const isConfirmed = window.confirm("Bạn có chắc chắn muốn xóa đánh giá này?");

        if (isConfirmed) {
            await deactiveFeedback(feedbackId);
                        ...
                        {feedbacks && feedbacks.map((feedback, index) => (
                            ...
                            >
                                ...
                                <td className="p-4 text-center">
                                    <FaTrash onClick={() => handleDeactiveFeedback(feedback.id)} className="text-red-500 cursor-pointer hover:scale-125 hover:text-red-600" title="Xem chi tiết" />
                                </td>
                            </tr>
                        ))}
                    </tbody>
