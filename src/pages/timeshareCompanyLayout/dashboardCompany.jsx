...
import { HiCurrencyDollar, HiUsers, HiOfficeBuilding, HiBriefcase } from "react-icons/hi"
import {
    Chart as ChartJS,
    CategoryScale,
    LinearScale,
...
import { getAllResort, getTotalMoney, getTotalResort, getTotalStaff, getAllWalletTransactions, getRevenueTSC } from "../../service/tsCompanyService/tsCompanyAPI";
...
const DashboardCompany = () => {
    ...
    const fetchTransactions = async () => {
        try {
            let data = await getAllWalletTransactions(pageTransactions, sizeTransactions);
            if (data.status === 200) {
    ...
    const processRevenueData = (revenue) => {
        const months = [
            ...
            "06-2024", "07-2024", "08-2024", "09-2024", "10-2024", "11-2024", "12-2024"
        ];
                ...
                >
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-3xl font-bold"><CountUp start={0} end={totalMoney} duration={3} /></p>
                        </div>
                    </div>
                </div>
                {/* Customer */}
                                    ...
                                    <div className="flex space-x-2 bg-gray-200 rounded-full px-2 py-1">
                                        {Array.from({ length: totalPagesTransactions }, (_, index) => {
                                            ...
                                            if (
                                                ...
                                                (index >= pageTransactions - 1 && index <= pageTransactions + 1)
                                            ) {
                                                return (
                                                    <button
                                                        key={index}
                                            ...
                                            if (
                                                (index === pageTransactions - 2 && pageTransactions > 2) ||
                                                (index === pageTransactions + 2 && pageTransactions < totalPagesTransactions - 3)
                                            ) {
                                                return (
                                                    <span
                                                        key={index}
