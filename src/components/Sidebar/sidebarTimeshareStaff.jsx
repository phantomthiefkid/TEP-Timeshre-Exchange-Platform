...
import { menuTimeshareStaff } from '../../util/MenuSidebarItem';
...
const SidebarTimeshareStaff = () => {
  ...
  const menuItemVariants = {
    hidden: { opacity: 0, scale: 0.8, x: -50 },  // Animation when item is hidden
    visible: (index) => ({
      opacity: 1,
      scale: 1,
      x: 0,
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 shadow-xl">
      {/* Sidebar */}
      <motion.div
        initial={{ width: '6rem' }}  // Initial width for sidebar
        animate={{ width: isOpen ? '18rem' : '6rem' }}  // Animate width change
        transition={{ duration: 0.5 }}  // Duration for sidebar toggle
      ...
      >
        ...
        <div>
          ...
          >
            {isOpen ? (
              <span className="text-lg font-bold text-gray-500">Timeshare staff</span>
            ) : (
              ...
              .map((item, index) => (
                ...
                >
                  <NavLink
                    to={item.path}
                    className={({ isActive }) =>
                      `flex items-center space-x-4 ml-4 p-3 text-sm rounded-l-2xl transition-all duration-75 ease-in-out hover:bg-blue-500 hover:text-gray-200
